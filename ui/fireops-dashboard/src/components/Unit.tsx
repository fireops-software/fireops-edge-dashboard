...
const Unit = ({unit}: {unit: UnitState}) => {
  return (
    <div className="h-24 w-48 flex m-4 text-white">
      <div className="w-1/5 h-full text-white flex flex-wrap" style={{ backgroundColor: unit.unit_status_color ?? "gray" }}>
        <span className="-rotate-90 translate-x-11">{unit.unit_status_id ? `(${unit.unit_status_id}) ` : "" }{unit.unit_status}</span>
      </div>
